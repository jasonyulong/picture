...
class Chart extends Think{
  ...
  */
  ...
  public function createMultiColumnar($title,$data=array(),$dataName=array(),$legend=array(),$filepath='',$size=40,$height=100,$width=80){  
	//设置多条柱状图
		...
		// Create a bar pot
		$colorArray = array('blue@0.3','red@0.3','green@0.3','brown@0.4','black@0.4','gray@0.4','yellow@0.4');
		$cc=0;
		$bplotArray = array();
		$dataC = count($data);
		for($y=0;$y<$dataC;$y++){
			if($y>6) $cc=0;
			$bplotNow = 'bplot'.$y;
			$$bplotNow = new BarPlot($data[$y]);
			$$bplotNow->SetFillColor($colorArray[$cc]);
			$$bplotNow->value->Show();
			$$bplotNow->value->SetFont(FF_ARIAL,FS_BOLD);
			$$bplotNow->value->SetAngle(90);
			$$bplotNow->value->SetColor($colorArray[$cc]);
			$$bplotNow->SetLegend($dataName[$y]);
			//$$bplotNow->SetShadow('black@0.4');
			$bplotArray[] = $$bplotNow;
			$cc++;
		}
		
		$graph->legend->SetFillColor('lightblue@0.9');
		$graph->legend->Pos(0.01,0.12,"right","center");
